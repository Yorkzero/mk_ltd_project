...
volatile static uint16_t ls_time_cnt = 0;//记录前次反射的时间
volatile static uint16_t ls_nxt_cnt = 0;//记录这次反射的时间
volatile static uint32_t motor_run_time = DEF_ROT_TIME;//记录此次电机转动时间
...
void user_app_run(void)
{
	u32 temp;//中间变量 防止两个volatile编译冲突
