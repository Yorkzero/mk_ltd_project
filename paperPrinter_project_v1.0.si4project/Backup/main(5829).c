...
volatile uint8_t ls_outtime_flag = 0;//判断光电开关检测是否超时的标志位
 uint16_t ls_outtime_cnt = 1000;//此处设定1s超时
 static uint16_t ls_time_cnt = 0;//记录前次反射的时间
 static uint16_t ls_nxt_cnt = 0;//记录这次反射的时间
 static uint32_t motor_run_time = DEF_ROT_TIME;//记录此次电机转动时间
